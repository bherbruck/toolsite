...
fn open(config: &Config) -> Result<Connection, String> {
    ...
    conn.execute_batch(
        "create table if not exists users (
         ...
         -- `scope` is null for a site session and an app's slug for a session
         -- that may only speak for that one app. Never read a session without
         -- saying which of the two you want.
         -- TODO(migrations): a database created before `scope` existed has no
         -- such column; the migration system needs to add it and backfill
         -- existing rows to null (they are all site sessions).
